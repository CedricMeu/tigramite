...
from tigramite.toymodels.context_model import (
    _shift_link_entries,
    ContextModel,
    _do_dummy_projection,
    _nb_latent_before,
)
...
def get_non_parents(links, tau_max):
    ...
    non_parents = [
        (var, tau)
        for var in links.keys()
        for tau in range(tau_max + 1)
        if (var, -tau) not in parents
    ]
    return non_parents
...
def a_generate_random_context_model(N=3, K_space=2, K_time=2, tau_max=2):
    # use seedsequence
    seed = 12347
    ...
    if K_time != 0:
        links_tc, noises_tc = toys.generate_structural_causal_process(
            len(nodes_tc),
            L_time,
            dependency_funcs,
            dependency_coeffs,
    ...
    if K_space != 0:
        ...
        links_sc, noises_sc = toys.generate_structural_causal_process(
            len(nodes_sc),
            L_space,
            dependency_funcs,
            dependency_coeffs,
    ...
    links_sys, noises_sys = toys.generate_structural_causal_process(
        len(nodes_sys),
        L,
        dependency_funcs,
        dependency_coeffs,
    ...
    context_sys_links = set_links(
        nodes_sys,
        L_context_sys,
        non_parent_sc + non_parent_tc,
        tau_max,
        dependency_funcs,
        dependency_coeffs,
...
@pytest.fixture(
    params=[
        ...
        (
            a_generate_random_context_model(N=3, K_space=2, K_time=2, tau_max=2),
            100,
            100,
            12346,
        ),
        (
            a_generate_random_context_model(N=5, K_space=2, K_time=2, tau_max=2),
            100,
            100,
            12347,
        ),
...
def a_jpcmciplus(request):
    ...
    contextmodel = ContextModel(
        links=links,
        node_classification=node_classification_gt,
        noises=noises,
        seed=seed_val,
    )
    ...
    time_context_indices = [
        node for node in links.keys() if node_classification_gt[node] == "time_context"
    ]
    space_context_indices = [
        node for node in links.keys() if node_classification_gt[node] == "space_context"
    ]
    system_indices = [
        node for node in links.keys() if node_classification_gt[node] == "system"
    ]
    ...
    observed_indices_time = random_state.choice(
        time_context_indices,
        size=int(percent_observed * len(time_context_indices)),
        replace=False,
    ).tolist()
    observed_indices_space = random_state.choice(
        space_context_indices,
    ...
    observed_spatial_context_nodes = list(
        range(N + len(observed_indices_time), N + nb_observed_context_nodes)
    )
    ...
    for node in observed_indices:
        node_classification[
            node
            - _nb_latent_before(node, observed_context_indices, node_classification_gt)
    ...
    augmented_links = _do_dummy_projection(
        links,
        node_classification_gt,
