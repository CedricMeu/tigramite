...
from pcmci.test_pcmci_calculations import a_chain, gen_data_frame
...
def test_linear_mediation_coeffs(data_frame_a):
    ...
    for j, i, tau, coeff in toys._iter_coeffs(links_coeffs):
        np.testing.assert_allclose(med.get_coeff(i=i, tau=tau, j=j), coeff, rtol=1e-1)
...
def test_predictions(data_frame_a):
    ...
    pred.fit(
        target_predictors=all_predictors,
        selected_targets=[target],
        tau_max=tau_max,
        return_data=True,
    )
