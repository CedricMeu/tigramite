...
def gen_data_sample(seed, corr_val, T):
    ...
    cov = np.array([[1.0, corr_val], [corr_val, 1.0]])
...
def check_get_array(ind_test, sample):
    ...
    a_array, a_xyz, a_xyz_nodes, _ = ind_test._get_array(x_nds, y_nds, z_nds, tau_max)
    # Get the array directly from the dataframe
    b_array, b_xyz, b_xyz_nodes, _ = dataframe.construct_array(
        x_nds,
        y_nds,
        z_nds,
        tau_max=tau_max,
...
def check_run_test(ind_test, sample):
    ...
    alpha_or_thres = 0.1
    # Run the test
    val, pval, dependent = ind_test.run_test(
        X=x_nds, Y=y_nds, Z=z_nds, tau_max=tau_max, alpha_or_thres=alpha_or_thres
    )
    ...
    if ind_test.significance == "fixed_thres":
        ...
        pval_expt = 0.0 if dependent else 1.0
...
def check_parcorr_wls_on_heteroskedastic_data(ind_test, sample):
    ...
    np.testing.assert_allclose(
        np.array(homoskedastic_val), np.array(val_est), atol=0.03
    )
...
def check_std_approximation(ind_test, sample, xlag, ylag):
    ...
    np.testing.assert_allclose(
        true_stds_shifted[2 * tau_max : -2 * tau_max, :].transpose(),
        np.array(val_est[:, 2 * tau_max : -2 * tau_max]),
        rtol=2.0,
    )
...
def test_parcorr_residuals(par_corr, seed):
    ...
    est_res = par_corr._get_single_residuals(
...
@pytest.fixture(
    params=[
        ...
        (
            ...
            None,
            {
                0: ["time-dependent heteroskedasticity"],
                1: ["time-dependent heteroskedasticity"],
            },
            False,
        ),
        (
...
def test_parcorr_wls_residuals(par_corr_wls, seed):
    ...
    array[0] += 0.5 * array[2:].sum(axis=0)
    # Estimate the residuals
    est_res = par_corr_wls._get_single_residuals(
        array, target_var, standardize=False, return_means=False
    )
    np.testing.assert_allclose(est_res, true_res[0], rtol=1e-5, atol=0.02)
...
def test_parcorr_wls_on_heteroskedastic_data(
    par_corr_wls_expert, data_sample_hs_parent
):
    # Get the data sample values
    check_parcorr_wls_on_heteroskedastic_data(
        par_corr_wls_expert, data_sample_hs_parent
    )


def test_parcorr_wls_on_heteroskedastic_data(
    par_corr_wls_expert_time, data_sample_hs_time
):
    # Get the data sample values
    check_parcorr_wls_on_heteroskedastic_data(
        par_corr_wls_expert_time, data_sample_hs_time
...
def test_gpdc_residuals(gpdc, seed):
    ...
    def func(x_arr, c_val=1.0):
        return c_val * x_arr * (1.0 - 4.0 * np.exp(-x_arr * x_arr / 2.0))
    ...
    (_, pred) = gpdc._get_single_residuals(
        array, target_var, standardize=False, return_means=True
    )
    # Testing that the fit matches in the centre
    cntr = np.where(np.abs(target_res) < 0.7)[0]
    np.testing.assert_allclose(pred[cntr], func(target_res[cntr], c_val), atol=0.2)
...
def test_trafo2uniform(gpdc, data_sample_a):
    ...
    for i in range(array.shape[0]):
        ...
        np.testing.assert_allclose(
            np.ones(bins) / float(bins), hist / float(bins), atol=0.01
        )
...
def test_gpdc_torch_residuals(gpdc_torch, seed):
    ...
    target_res = np.copy(array[2])
    # Calculate the residuals
    (_, pred) = gpdc_torch._get_single_residuals(
        array, target_var, standardize=False, return_means=True
    )
    # Testing that the fit matches in the centre
    cntr = np.where(np.abs(target_res) < 0.7)[0]
    np.testing.assert_allclose(
        pred[cntr], func(target_res[cntr], c_val), atol=0.2, rtol=1e-01
    )
...
def test_cmi_knn(cmi_knn, data_sample_c):
    ...
    np.testing.assert_allclose(
        np.array(_par_corr_to_cmi(corr_val)), np.array(val_est), atol=0.02
    )
