...
class RPCMCI(PCMCI):
    ...
    def run_rpcmci(
        ...
        def _pcmci(tau_min, tau_max, pc_alpha, alpha_level):
            ...
            graph = results["graph"]
            pcmci_parents = self.return_parents_dict(
                graph=graph, val_matrix=results["val_matrix"]
            )
        ...
        def _optimize_gamma(resid_sq, max_transitions):
            ...
            E = [
                solver.NumVar(0, infinity, f"eta_{i}")
                for i in range(num_regimes * T - 1)
            ]
            X = G + E
            solver.Minimize(
                sum(
                    [
                        resid_sq[k, t] * X[k * T + t]
            ...
            con_lst = [
                sum([X[k * T + t] for k in range(num_regimes)]) for t in range(T)
            ]
            for t in range(T):
            ...
            if status == pywraplp.Solver.OPTIMAL:
                ...
                if self.verbosity > -1:
                    print(
                        "The problem does not have an optimal solution. Please change hyperparameters."
                    )
                exit(0)
        ...
        def one_annealing_step(a):
            ...
            gamma_opt = random_state.uniform(
                0, 1, size=(num_regimes, T)
            )  # range is [0,1)!
            ...
            for q in range(num_iterations):
                ...
                for k in range(num_regimes):
                    ...
                    if np.any((mask_of_k == False).sum(axis=0) <= 5):
                        ...
                        if self.verbosity > -1:
                            print(
                                f"*****Regime with too few samples in annealing a = {a} at iteration q = {q}.*****\n"
                            )
                        if self.verbosity > -1:
                    ...
                    try:
                        ...
                        error_flag = True
                        print(
                            f"*****Value error in causal discovery for annealing a = {a} at iteration q = {q}.*****\n"
                        )
                        print("***** Break k-loop of regimes *****\n ")
                        break  # from k-loop
                    ...
                    try:
                        ...
                        for target in range(self.N):
                            ...
                            original_data[:, target] = pred.get_test_array(target)[
                                0
                            ].flatten()
                        ...
                        error_flag = True
                        print(
                            f"*****Value error in prediction for annealing a = {a} at iteration q = {q}.*****\n"
                        )
                        print("***** Break k-loop of regimes *****\n ")
                        break  # from k-loop
                ...
                if error_flag:
                    if self.verbosity > -1:
                        print(
                            f"***** Break q-loop of optimization iterations for Annealing a = {a} at iteration q = {q}."
                ...
                try:
                    ...
                    error_flag = True
                    print(
                        f"*****Value error in optimization for annealing a = {a} at iteration q = {q}.*****\n"
                    )
                    break
                ...
                if self.verbosity > -1:
                    print(
                        f"Difference in abs value between the previous and current gamma "
                ...
                if diff_g[-1] == 0:
                    if self.verbosity > -1:
                        print(
                            "Two consecutive gammas are equal: (local) minimum reached. "
                            "Go to next annealing.\n"
                        )
                    break

                if (q >= q_break_cycle) and (
                    diff_g[-1] <= (2 * num_regimes * T // 100)
                ):
                    if self.verbosity > -1:
                        print(
                            f"Iteration larger than {q_break_cycle} and two consecutive gammas are too similar. "
