...
from tigramite.pc import PC, run_cond_ind_test
from multiprocessing import Pool

import numpy as np


class PCParallelInner(PC):
    def run(
        self,
        selected_links=None,
        ...
        self.pcmci._check_tau_limits(tau_min, tau_max)
        ...
        #                                      remove_contemp=True)
        _int_link_assumptions = self.pcmci._set_link_assumptions(
        ...
        max_conds_dim = self.pcmci._set_max_condition_dim(
            max_conds_dim, tau_min, tau_max
        )

        with Pool() as pool:
            # Loop through the selected variables
            for j in range(self.pcmci.N):
        ...
        self.val_matrix = self._dict_to_matrix(
            val_dict, tau_max, self.pcmci.N, default=0.0
        )
        self.p_matrix = self._dict_to_matrix(
            pval_max, tau_max, self.pcmci.N, default=1.0
        )
    ...
    def __run_single(
        ...
        for conds_dim in range(max_conds_dim + 1):
            ...
            for (parent, *_state_rest), _result in zip(
                state, pool.starmap(run_cond_ind_test, args)
            ):
