...
def calculate(para_setup):
    ...
    if model == "autobidirected":
        ...
        auto_deps = list(np.arange(max(0.0, auto - 0.3), auto + 0.01, 0.05))
        ...
        for ir in range(1000):
            ...
            N_all = math.floor((N / (1.0 - frac_unobserved)))
            n_links_all = math.ceil(n_links / N * N_all)
            observed_vars = np.sort(
                random_state.choice(
                    range(N_all),
                    size=math.ceil((1.0 - frac_unobserved) * N_all),
                    replace=False,
                )
            ).tolist()
            ...
            class noise_model:
                ...
                def weibull(self, T):
                    ...
                    a = 2
                    mean = scipy.special.gamma(1.0 / a + 1)
                    variance = (
                        scipy.special.gamma(2.0 / a + 1)
                        - scipy.special.gamma(1.0 / a + 1) ** 2
                    )
                    return (
                        self.sigma
                        * (random_state.weibull(a=a, size=T) - mean)
                        / np.sqrt(variance)
                    )
                ...
                def uniform(self, T):
                    ...
                    return (
                        self.sigma
                        * (random_state.uniform(size=T) - mean)
                        / np.sqrt(variance)
                    )
            ...
            if "discretebinom" in model:
                ...
                data_all_check, nonstationary = discretized_scp(
                    links=links, T=T + 10000, n_binom=n_binom, random_state=random_state
                )
            else:
                data_all_check, nonstationary = (
                    mod.generate_nonlinear_contemp_timeseries(
                        links=links,
                        T=T + 10000,
                        noises=noises,
                        random_state=random_state,
    ...
    true_graph = utilities._get_pag_from_dag(
        links, observed_vars=observed_vars, tau_max=tau_max, verbosity=verbosity
    )[1]
...
if __name__ == "__main__":
    ...
    for conf in list(all_configs.keys()):
        all_configs[conf]["graphs"] = np.zeros(
            (samples,) + all_configs[conf]["results"][0]["graph"].shape, dtype="<U3"
        )
        all_configs[conf]["true_graphs"] = np.zeros(
            (samples,) + all_configs[conf]["results"][0]["true_graph"].shape,
            dtype="<U3",
        )
        all_configs[conf]["val_min"] = np.zeros(
            (samples,) + all_configs[conf]["results"][0]["val_min"].shape
        )
        ...
        for i in list(all_configs[conf]["results"].keys()):
            ...
            all_configs[conf]["true_graphs"][i] = all_configs[conf]["results"][i][
                "true_graph"
            ]
            all_configs[conf]["val_min"][i] = all_configs[conf]["results"][i]["val_min"]
            all_configs[conf]["max_cardinality"][i] = all_configs[conf]["results"][i][
                "max_cardinality"
            ]

            all_configs[conf]["computation_time"].append(
                all_configs[conf]["results"][i]["computation_time"]
            )
        ...
        for metric in metrics:
            if metric != "computation_time":
                print(
                    f"{metric:30s} {metrics[metric][0]: 1.2f} +/-{metrics[metric][1]: 1.2f} "
                )
            else:
                print(
        ...
        print(
            "Metrics dump ",
            file_name.replace("'", "").replace('"', "") + "_metrics.dat",
        )
    ...
    time_end = time.time()
    print("Run time in hours ", (time_end - time_start) / 3600.0)
